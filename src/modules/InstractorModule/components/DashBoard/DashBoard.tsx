...
import { QUIZ,STUDENT } from "../../../../config/api.endPoint";
...
export default function DashBoard() {
  ...
  const formatDate = (dateString:string) => {
   ...
   const getFiveIncomingQuizzes = async ()=>{
    try {
      const response = await axiosInstance.get(QUIZ.FIRST_INCOMMING);
   ...
   const topFiveStudents = async()=>{
    try {
      const response = await axiosInstance.get(STUDENT.TOP_FIVE_STUDENTS);
      console.log(response.data)
  ...
  return (
    <>
   <div className="grid grid-cols-2 gap-4 mx-3 my-5">
